...
const file = {
  ...
  load_lines: (f) =>
    file
      ...
      .filter((v) => v),
  save_lines: (f, a) =>
    file.save(f, a.filter((v) => v).join(file.EOL) + file.EOL),
  age_days: (f) =>
    file.exists(f)
      ? (Date.now() - fs.statSync(f).ctimeMs) / (1000 * 60 * 60 * 24)
      : null,
  is_file: (f) => file.exists(f) && file.stat(f).isFile(),
  is_dir: (f) => file.exists(f) && file.stat(f).isDirectory(),
  ls: (f) =>
    file.is_dir(f) ? fs.readdirSync(f).map((ff) => file.resolve(f, ff)) : [],
  ls_files: (f) => file.ls(f).filter(file.is_file),
  mkdir: (f) =>
    file.is_dir(f) || (!file.exists(f) && fs.mkdirSync(f, { recursive: true }))
      ? f
      : "",
  mkfile: (f) =>
    file.is_file(f) || (!file.exists(f) && !fs.closeSync(fs.openSync(f, "a"))),
