...
const { mode_fail, is, must } = require("./lib_is");
mode_fail();

// returns a link object
function Link({ keyfile = "", project = "", dataset = "", table = "" }) {
...
function connect({ keyfile = "", project = "", dataset = "", table = "" }) {
  return new Link({ keyfile, project, dataset, table });
}
...
async function table_create(link) {
  ...
  await link.bq
    ...
    .createTable(link.table, { schema: link.schema });
...
async function table_schema(link) {
  ...
  if (await table_exists(link)) {
    return (
      await link.bq.dataset(link.dataset).table(link.table).getMetadata()
    )[0].schema;
  }
}
...
async function list_tables(link, dataset = "") {
  is(Link, link);
  is(String, dataset);
  return (await link.bq.dataset(dataset || link.dataset).getTables())[0].map(
    (v) => v.id,
  );
}
async function list(link) {
  ...
  for (let d of ds) {
    res[d] = await list_tables(link, d);
  }
  return res;
}
...
function query_download(link, ofile, sql) {
  ...
  return query_stream(link, sql, (v) =>
    ostream.write(JSON.stringify(v) + "\n"),
  );
}
...
function query_finalise(sql = "", replace = false) {
  return sql.constructor === Array
    ? sql.map((s) => query_finalise(s, replace))
    : sql
        ...
        .join("\n") // remove blank lines
        .replaceAll(
          // create or replace       OR   create only if missing
          replace ? /CREATE TABLE IF NOT EXISTS/gi : "CREATE OR REPLACE TABLE",
          replace ? /CREATE OR REPLACE TABLE/gi : "CREATE TABLE IF NOT EXISTS",
        );
}
...
function download(link, ofile = "./out.jsonl", format = "jsonl") {
  switch (format) {
    case "jsonl":
      download_jsonl(link, ofile);
      break;
    case "csv":
      download_csv(link, ofile);
      break;
    default:
      console.error(__filename, "unknown format", format);
  }
}
function download_jsonl(link, ofile = "./out.jsonl") {
  ...
  return query_stream(
    link,
    `SELECT * FROM \`${link.dataset}.${link.table}\``,
    (v) => ostream.write(JSON.stringify(v) + "\n"),
  );
}
    ...
    for (let { name, type, mode, description, fields } of field_list) {
      ...
      if (mode == "REPEATED") {
        ...
        if (type == "RECORD") {
          ...
          sql.push(
            `${indent}${name} ${type}${mode == "REQUIRED" ? " NOT NULL" : ""} ${description},`,
          );
        }
      }
    }
...
async function create_from_bucket({
  link = {},
  name = "a_table",
  bucket = "",
  ...
  if (replace || !(await table_exists(link))) {
    try {
      link.schema = JSON.parse(
        execSync(
          `curl https://storage.googleapis.com/${bucket}/${path_schema}`,
        ),
      );
    } catch (e) {
