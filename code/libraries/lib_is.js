...
function tname(t) {
  return t?.name ?? (t === null ? "null" : "undefined");
}
function vname(v) {
  return v?.constructor?.name ?? (v === null ? "null" : "undefined");
}
function vtype(v) {
  return v?.constructor ?? (v === null ? null : undefined);
}

function print_trace(msg) {
  console.error(
    [
      "",
      msg,
      ...
      ...new Error().stack
        .split("\n")
        .slice(2)
        .filter((s) => !s.includes(__filename)),
    ].join("\n"),
  );
  return false;
}
...
function check(t, v) {
  return check_bool(t, v);
}
function check_bool(t, v) {
  return t === vtype(v);
}
function check_warn(t, v) {
  return (
    check_bool(t, v) ||
    print_trace(
      `ERROR: A value of type '${vname(v)}' was received. It should be of type '${tname(t)}'`,
    )
  );
}
function check_fail(t, v) {
  return check_warn(t, v) || process.exit(1);
}
...
function may(bool, msg = "") {
  return bool || !!print_trace(msg || `ERROR: assertion failed`);
}
function must(bool, msg = "") {
  return may(bool, msg) || process.exit();
}
...
if (require.main === module) {
  const test = (res) =>
    res ? console.log("test passed") : print_trace("test failed");
  ...
  heading(
    "Testing warning mode (you should see ERROR: messages and stack traces)",
  );
  mode_warn();
  test(is(Number, 5));
  test(!is(String, 5));
