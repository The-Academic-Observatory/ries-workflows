...
BEGIN 
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.core_heps${version}\` AS (
    SELECT 
      A.ror,
      B.name,
      A.name AS era_name
    FROM \`${project}.${dataset}.raw_heps${version}\` AS A
    LEFT JOIN \`${project}.${dataset}.core_rors${version}\` AS B ON A.ror = B.ror
    ORDER BY era_name
  );
  ALTER TABLE \`${project}.${dataset}.core_heps${version}\` SET OPTIONS(description='Australian higher education providers (list provided by ERA 2018)');
  ALTER TABLE ${project}.${dataset}.core_heps${version}
  ...
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.core_heps_auto${version}\` AS (
    SELECT 
      ror,
      name,
    FROM ${project}.${dataset}.core_rors${version}
    WHERE 
  ...
  ALTER TABLE \`${project}.${dataset}.core_heps_auto${version}\` SET OPTIONS(description='Australian education providers (filtered by country and activity type)');
  ALTER TABLE ${project}.${dataset}.core_heps_auto${version}
  ALTER COLUMN ror  SET OPTIONS (description="Institutional Research Organisations Registry code (see https://ror.org"),
  ...
  FROM ${project}.${dataset}.core_heps${version};
  SELECT
    'there should be no nulls or empty strings' AS description,
    IF(COUNTIF(ror  IS NULL OR ror  = '') = 0, 'pass', 'fail') AS test1,
    IF(COUNTIF(name IS NULL OR name = '') = 0, 'pass', 'fail') AS test2,
  FROM ${project}.${dataset}.core_heps_auto${version};
