...
BEGIN 
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.core_rors${version}\` AS (
    SELECT 
      ror,
      since,
      status ,
      ...
      types,
      links  
    FROM ${project}.${dataset}.raw_rors${version}
    ORDER BY name
  );
  ALTER TABLE \`${project}.${dataset}.core_rors${version}\` SET OPTIONS(description='Official Research Organisation Registry data sourced from https://ror.readme.io/docs/data-dump');
  ALTER TABLE ${project}.${dataset}.core_rors${version}
