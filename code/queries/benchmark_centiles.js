...
BEGIN
  ...
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.benchmarks_centiles_${scope}_${digits}_${version}\` AS (
    SELECT DISTINCT 
      F.code AS field, 
  ...
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.centile_tallies_${scope}_${digits}_${version}\` AS (
    SELECT 
      F.code AS field,
      year_published AS year,
    ...
    FROM \`${project}.${dataset}.core_papers${version}\` AS A
    LEFT JOIN UNNEST(fors) AS F
    LEFT JOIN \`${project}.${dataset}.benchmarks_centiles_${scope}_${digits}_${version}\` AS B ON F.code = B.field AND year_published = B.year
    WHERE LENGTH(F.code) >= ${digits} ${scope == "local" ? "AND ARRAY_LENGTH(heps) > 0" : ""}
    GROUP BY field,year 
    ORDER BY field,year 
  );
...
-- generated by: ${require("path").basename(__filename)}
BEGIN
  -- citation centile boundaries
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.benchmarks_centiles${version}\` AS (
    SELECT DISTINCT 
      F.code AS field, 
      year_published AS year, 
      PERCENTILE_DISC(num_citations, 0.99 IGNORE NULLS) OVER(PARTITION BY F.code, year_published) AS c1, 
  ...
  -- gather statistics for each centile group
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.centile_tallies${version}\` AS (
    SELECT 
      F.code AS field,
      year_published AS year,
    ...
    FROM \`${project}.${dataset}.core_papers${version}\` AS A
    LEFT JOIN UNNEST(fors) AS F
    LEFT JOIN \`${project}.${dataset}.benchmarks_centiles${version}\` AS B ON F.code = B.field AND year_published = B.year
    GROUP BY field,year 
    ORDER BY field,year 
  );
END;
