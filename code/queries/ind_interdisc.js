...
## Description
For methodology, see:
https://github.com/Curtin-Open-Knowledge-Initiative/coki-ries/blob/main/docs/era_2018.md#indicator-interdisciplinary-profiles
...
BEGIN
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.interdisc_${scope}_${digits}_${version}\` AS (
    WITH 
    codes AS (
      SELECT 
        A.doi,
      ...
      LEFT JOIN \`${project}.${dataset}.core_fors${version}\` AS C ON SUBSTRING(B.code,0,2) = C.code
...
-- generated by: ${require("path").basename(__filename)}
BEGIN
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.interdisc${version}\` AS (
    WITH 
    codes AS (
      SELECT 
        A.doi,
        ...
        C.name
      FROM \`${project}.${dataset}.core_papers${version}\` AS A 
      LEFT JOIN UNNEST(concs) AS B
      LEFT JOIN \`${project}.${dataset}.core_fors${version}\` AS C ON B.code = C.code
    ),
    pairs AS (
      SELECT 
        A.doi,
