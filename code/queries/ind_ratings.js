...
BEGIN
  ...
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.rci_scores_${scope}_${digits}_${version}\` AS ( 
    WITH ror_for_year AS (
      ...
      FROM       \`${project}.${dataset}.paper_x_hep${version}\`     AS A 
      INNER JOIN \`${project}.${dataset}.forcode_x_paper${version}\` AS B ON A.doi = B.doi AND B.vers = 2020 AND LENGTH(code) = ${digits} -- filter for codes
      INNER JOIN \`${project}.${dataset}.papers${version}\`          AS C ON A.doi = C.doi AND year_published BETWEEN ${start} AND ${finish} -- filter time frame
    ...
    with_benchmarks AS (
      ...
      LEFT JOIN \`${project}.${dataset}.core_heps${version}\`            AS B ON A.ror  = B.ror
      LEFT JOIN \`${project}.${dataset}.core_fors${version}\`            AS D ON A.vers = D.vers AND A.code = D.code
      LEFT JOIN \`${project}.${dataset}.benchmarks_hpi${version}\`       AS C ON A.vers = C.vers AND A.code = C.code AND A.year = C.year 
      LEFT JOIN \`${project}.${dataset}.rci_benchmarks_world${version}\` AS E ON A.vers = E.vers AND A.code = E.code AND A.year = E.year_published
    )
    SELECT 
      *,
  ...
  -- ALTER TABLE ${project}.${dataset}.ind_ratings${version}
  -- ALTER COLUMN ror
  -- ALTER COLUMN name
  ...
  */
  ...
  -- assign ratings
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.ratings_${scope}_${digits}_${version}\` AS (
    SELECT 
      institution,
      field,
      year,
      ...
      END AS rating_world,
      CASE
        -- consider easing the top HPI limit to 1.4
    ...
    FROM ${project}.${dataset}.rci_grouping_${scope}_${digits}_institution_field_year${version}
...
BEGIN
  /* currently inactivated
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.rci_scores${version}\` AS ( 
    WITH ror_for_year AS (
      SELECT 
        A.ror,
        B.vers,
        ...
        ROUND(AVG(num_citations),4) AS avg_citations 
      FROM       \`${project}.${dataset}.paper_x_hep${version}\`     AS A 
      INNER JOIN \`${project}.${dataset}.forcode_x_paper${version}\` AS B ON A.doi = B.doi AND B.vers = 2020
      INNER JOIN \`${project}.${dataset}.papers${version}\`          AS C ON A.doi = C.doi AND year_published BETWEEN ${start} AND ${finish} -- filter time frame
      GROUP BY ror, vers, code, year 
      ORDER BY num_citations DESC
    ),
    ...
    with_benchmarks AS (
      ...
      LEFT JOIN \`${project}.${dataset}.core_fors${version}\`            AS D ON A.vers = D.vers AND A.code = D.code
      LEFT JOIN \`${project}.${dataset}.benchmarks_hpi${version}\`       AS C ON A.vers = C.vers AND A.code = C.code AND A.year = C.year 
      LEFT JOIN \`${project}.${dataset}.rci_benchmarks${version}\` AS E ON A.vers = E.vers AND A.code = E.code AND A.year = E.year_published
    )
    SELECT 
      *,
      IF(cpp IS NULL OR hpi_benchmark IS NULL OR hpi_benchmark = 0, 0, ROUND(cpp / hpi_benchmark,2)) AS relative_hpi,
  ...
  */
  ...
  -- assign ratings
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.ratings${version}\` AS (
    SELECT 
      inst,
      conc,
      year,
      rci_world,
      CASE
        WHEN rci > 1.6 THEN '5 - well above world standard'
        WHEN rci > 1.2 THEN '4 - above world standard'
        WHEN rci > 0.8 THEN '3 - around world standard'
        WHEN rci > 0.4 THEN '2 - below world standard'
        WHEN rci > 0   THEN '1 - well below world standard'
        ELSE                'NA - not assessable'
      END AS rating_world,
      CASE
        WHEN hpi > 1.6 AND rci > 1.6 THEN '5 - world leading'
        WHEN hpi > 0.8 AND rci > 1.6 THEN '4 - well above world standard'
        WHEN hpi < 0.8 AND rci > 1.2 THEN '3 - above world standard' 
        WHEN rci > 0.8               THEN '2 - world standard'
        WHEN rci > 0                 THEN '1 - not at world standard'
        ELSE                              'NA - not assessable'
      END AS rating_option_a,
      CASE
        WHEN hpi > 1.6 THEN 'AAA - world leading'
        WHEN hpi > 1.2 THEN 'AA - above high performance standard'
        WHEN hpi > 0.8 THEN 'A - around high performance standard'
        WHEN rci > 1.2 THEN 'B - above world standard'
        WHEN rci > 0.8 THEN 'C - around world standard'
        WHEN rci > 0   THEN 'D - below world standard'
        ELSE                'NA - not assessable'
      END AS rating_option_b
    FROM ${project}.${dataset}.rci_grouping_inst_conc_year${version}
    ORDER BY rci DESC 
  );
END;
`;
