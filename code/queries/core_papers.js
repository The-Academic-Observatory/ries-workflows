...
- were published between a start and end year
...
- filtered list of ISSNs
- filtered list of linking ISSNs (should be one)
...
BEGIN 
  ...
  CREATE TEMP TABLE base_papers AS (
    SELECT 
      ...
      issns, 
      rors 
    FROM ${project}.${dataset}.raw_papers${version}
    WHERE 
      doi IS NOT NULL AND doi != ''  -- must be an identified paper
      AND year BETWEEN ${start} AND ${finish} -- must be published within the ERA period
  ...
  CREATE TEMP TABLE filter_journals AS (
    ...
    INNER JOIN (
      ...
      FROM ${project}.${dataset}.core_journals${version}
      LEFT JOIN UNNEST(issns) AS issn
    ) AS B
    ON issn = B.issn
    GROUP BY doi
  ...
  CREATE TEMP TABLE filter_rors AS (
    ...
    INNER JOIN \`${project}.${dataset}.core_rors${version}\` AS X ON ror = X.ror
    GROUP BY doi
  );
  ...
  CREATE TEMP TABLE filter_heps AS (
    ...
    FROM filter_rors
    LEFT JOIN UNNEST(rors) AS ror
    INNER JOIN \`${project}.${dataset}.core_heps${version}\` AS X ON ror = X.ror 
    GROUP BY doi
  );
  ...
  --   FROM base_papers
  --   LEFT JOIN UNNEST(issns) AS issn 
  --   INNER JOIN \`${project}.${dataset}.xref_issn_issnl${version}\` AS X ON issn = X.issn 
  --   GROUP BY doi
  -- );
  ...
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.core_papers${version}\` AS (
    SELECT A.*, B.fors
    FROM final_filtered AS A
    INNER JOIN \`${project}.${dataset}.core_journals${version}\` AS B ON A.era_id = B.era_id AND ARRAY_LENGTH(B.fors) > 0
    ORDER BY year_published, num_citations DESC
  );
  SELECT 'final number of papers', COUNT(1) AS total, COUNT(DISTINCT doi) AS uniques FROM ${project}.${dataset}.core_papers${version};
  
  -- set the schema
  ALTER TABLE \`${project}.${dataset}.core_papers${version}\` SET OPTIONS(description='A subset of the COKI DOI dataset that only includes journal articles relevant to ERA analysis');
  ALTER TABLE ${project}.${dataset}.core_papers${version}
  ...
  FROM ${project}.${dataset}.core_papers${version};
END;
`;
