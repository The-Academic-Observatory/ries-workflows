...
BEGIN
  ...
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.benchmarks_hpi_world_${digits}_${version}\` AS (
    ...
    WITH active_institutions AS (
      ...
      FROM \`${project}.${dataset}.research_outputs_world_${digits}_institution_field${version}\`
    ...
    FROM (
      ...
      FROM \`${project}.${dataset}.research_outputs_world_${digits}_institution_field_year${version}\` AS A
  ...
  ALTER TABLE \`${project}.${dataset}.benchmarks_hpi_world_${digits}_${version}\`
...
BEGIN
  -- build the High Performance Indicator benchmark table
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.benchmarks_hpi_world${version}\` AS (
    
    -- filter for active institutions (>= ${threshold} outputs in a field)
    WITH active_institutions AS (
      SELECT inst, conc, sum_papers, sum_citations
      FROM \`${project}.${dataset}.research_outputs_inst_conc${version}\`
      WHERE sum_papers >= ${threshold}
    ),
    ...
    SELECT
      field,
      year,
      COUNT(DISTINCT inst)                 AS num_insts,
      SUM(sum_papers)                      AS sum_papers,
      SUM(sum_citations)                   AS sum_citations,
      SUM(sum_citations) / SUM(sum_papers) AS benchmark
    FROM (
      SELECT 
        ...
        A.avg_citations,
        PERCENT_RANK() OVER (PARTITION BY A.field, A.year ORDER BY A.avg_citations DESC) AS pct_rank
      FROM \`${project}.${dataset}.research_outputs_inst_conc_year${version}\` AS A
      INNER JOIN active_institutions AS B ON A.inst = B.inst
      INNER JOIN active_fields       AS C ON A.conc = C.conc
    )
    WHERE pct_rank <= 0.1
    GROUP BY conc,year
    ORDER BY conc,year
  );
  ALTER TABLE \`${project}.${dataset}.benchmarks_hpi${version}\`
  ALTER COLUMN field            SET OPTIONS (description="Field of Research (FoR) code"),
  ALTER COLUMN year             SET OPTIONS (description="Year of consideration (research outputs published in this year)"),
  ALTER COLUMN num_institutions SET OPTIONS (description="The number of high performing institutions that made it into the top 10%"),
  ALTER COLUMN sum_papers       SET OPTIONS (description="The number of papers published by the top 10% of institutions"),
  ALTER COLUMN sum_citations    SET OPTIONS (description="The total number of citations for outputs published by the top 10% of institutions"),
