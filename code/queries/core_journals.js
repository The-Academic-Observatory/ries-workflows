...
BEGIN
  ...
  CREATE TEMP TABLE raw_journals AS (
    -- SELECT '2018' as era_round, * FROM \`${project}.${dataset}.raw_journals_2018_${version}\` UNION ALL
    SELECT '2023' as era_round, * FROM \`${project}.${dataset}.raw_journals_2023_${version}\` 
  );
  ...
  --   LEFT JOIN \`${project}.${dataset}.xref_issn_issnl${version}\` AS B ON issn = B.issn
  --   LEFT JOIN \`${project}.${dataset}.xref_issn_issnl${version}\` AS C ON issn = C.issnl
  ...
  CREATE TEMP TABLE filtered_codes AS (
    ...
    FROM (
      ...
      LEFT JOIN \`${project}.${dataset}.core_fors${version}\` AS B ON fc.code = B.code
  ...
  ${replace ? "CREATE OR REPLACE TABLE" : "CREATE TABLE IF NOT EXISTS"} \`${project}.${dataset}.core_journals${version}\` AS (
    SELECT 
      A.era_round,
      A.era_id,
      A.title,
  ...
  ALTER TABLE \`${project}.${dataset}.core_journals${version}\` SET OPTIONS(description="The ERA Journal List after filtering (see also raw_journals)");
  ALTER TABLE ${project}.${dataset}.core_journals${version}
  ...
  FROM \`${project}.${dataset}.raw_journals_2018_${version}\` AS A
  LEFT JOIN \`${project}.${dataset}.core_journals${version}\` AS B ON A.era_id = B.era_id AND B.era_round = '2018'
  WHERE B.era_id IS NULL;

  SELECT 'dropped journal 2023', A.*
  FROM \`${project}.${dataset}.raw_journals_2023_${version}\` AS A
  LEFT JOIN \`${project}.${dataset}.core_journals${version}\` AS B ON A.era_id = B.era_id AND B.era_round = '2023'
  WHERE B.era_id IS NULL;
